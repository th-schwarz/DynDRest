...
class IntegrationTest extends GenericIntegrationTest {
  ...
  final void testBasicAuth_log() throws Exception {
    mockMvc
        .perform(get("/log").with(httpBasic("log-dev", "l0g-dev")))
        //			.andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(new MediaType("text", "html", StandardCharsets.UTF_8)))
  ...
  final void testBasicAuth_global() throws Exception {
    mockMvc
        .perform(
            get("/info/update-log").queryParam("page", "0").with(httpBasic("dyndns", "test123")))
        //			.andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(
            content()
                .string(startsWith("{\"total\":38,\"totalPage\":10,\"page\":1,\"pageSize\":4")));
  }
