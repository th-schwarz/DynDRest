...
- The JSON-API of https://www.cloudflare.com/[Cloudflare]. +
  Implemented by https://gitlab.com/thischwa/CloudflareDNS-java.git[CloudflareDNS-java].
...
* **Immediate Mode** (default): IP updates are processed instantly when requested via the API.
* **Scheduled Mode**: IP updates are queued and processed in batches by a background scheduler.

The scheduled mode is enabled by setting the `dyndrest.zone-update-interval-seconds` property to a positive integer value (in seconds). When this property is configured, DynDRest will:
...
- Process all queued updates in batches at the specified interval
...
----
dyndrest:
  zone-update-interval-seconds: 60  # Process updates every 60 seconds
----

To disable the scheduler and return to immediate processing, simply remove or comment out this property:

[source,yaml]
----
dyndrest:
  # zone-update-interval-seconds: 60  # Disabled - immediate processing
----
