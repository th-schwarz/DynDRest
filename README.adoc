...
release (latest by date including pre-releases)]]    image:https://img.shields.io/github/downloads/th-schwarz/DynDRest/dyndrest.jar[GitHub Downloads (specific asset, all releases)]
...
│   ├── dyndrest.yml
│   ├── dyndrest-0.9.0.jar
│   ├── dyndrest.jar -> dyndrest-0.9.0.jar
...
* [/manage/health]: A very basic health check with an extra basic-auth user. It can be enabled and configured by setting `management.endpoint.health.enabled=true` and the both properties `dyndrest.healthcheck-user-name` and `dyndrest.healthcheck-user-password`.
